...
const Wire = class extends EventEmitter {
	constructor({requestRejected = false} = {}) {
		super();

		this._requestRejected = requestRejected;
	...
	request(pieceIndex, offset, chunkSize, callback) {
		if (this._requestRejected) {
			setImmediate(() => {callback('Wire error', null)})
		} else {
