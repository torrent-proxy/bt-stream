...
const withLogs = (name, callback) => (...args) => {
	// console.log('peer-wire-swarm::Call::', name);
...
var toAddress = withLogs('toAddress', function(wire) {
...
var onwire = withLogs('onwire', function(swarm, connection, onhandshake, isServer) {
...
var leave = withLogs('leave', function(port, swarm) {
	if (!pools[port]) {
		return;
	}
	...
	if (Object.keys(pools[port].swarms).length) {
		return;
	}
...
var join = withLogs('join', function(port, swarm) {
	var pool = pools[port];
	...
	if (!pool) {
		...
		if (swarm.utp) {
			servers.push(utp.createServer(onconnection));
		}
...
var Swarm = function(infoHash, peerId, options) {
	if (!(this instanceof Swarm)) {
		return new Swarm(infoHash, peerId, options);
	}
...
Swarm.prototype.priority = function(addr, level) {
	...
	if (typeof level !== 'number' || peer.priority === level) {
		return level;
	}

	if (!this._queues[level]) {
		this._queues[level] = fifo();
	}
...
Swarm.prototype._shift = function() {
	for (var i = this._queues.length-1; i >= 0; i--) {
		if (this._queues[i] && this._queues[i].length) {
