...
const Swarm = class extends EventEmitter {
	...
	_onWire(wire) {
		log(`_onWire`);
		if (!this._bitfield) {
			log.debug('_onWire::exchangeMetadata');
			exchangeMetadata({infoHash: this._hash}, (metadata) => {
				log.debug('exchangeMetadata::inside callback');
	...
	_wirePreprocessing({wire, port, bitfield}) {
		...
		wire.on(`unchoke`, () => {
			...
			if (this._wires.some(it => it._wire === wire)) {
				return;
			}
