...
const {Wire, Status: WireStatus} = require(`../wire/wire`);
...
const Swarm = class extends EventEmitter {
	constructor({dht, peerSwarm, port, hash, swarm}) {
		super();
		...
		if (swarm) {
			this._swarm = swarm;
			this._swarm.on(this._swarm.EVENT_ADDED_WIRE, (event, wire) => {
				console.log(`add additional wire`, event)
				this._onWire(wire);
			});
		}
