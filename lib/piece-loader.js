...
const loadBlock = ({wire, offset, pieceIndex, chunkSize}) => {
	return new Promise((resolve, reject) => {
		wire.status = WireStatus.BUSY;

		const timeoutId = setTimeout(() => reject('timeout'), 1000 * 3);

		wire.wire.request(pieceIndex, offset, chunkSize, (err, block) => {
...
module.exports = class PieceLoader {
	...
	_work() {
		...
		freePieces.forEach((chunkIndex, i) => {
			...
			return loadBlock({wire: freeWire, offset, pieceIndex: this._pieceIndex, chunkSize: this._chunkSize})
				.then((block) => {
					this._piece[chunkIndex] = block;
