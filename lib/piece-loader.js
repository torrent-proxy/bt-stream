	...
	async _work() {
		...
		for (const chunkIndex of freeChunksIndex) {
			...
			loadChunk({wire: freeWire, offset, pieceIndex: this._pieceIndex, chunkSize})
				...
				.catch((err) => {
					log.warn(`FAILED: id`, freeWire._id, `err`, err, `wires.length`, this._wires.length);
					...
					log.debug('Chunk map after error', createChunksMap(this._chunksCount, this._piece));
